...
long long re_init(int node, int start, int end, int target)
{
    ...
    if (target > mid) // 찾는 위치가 중앙보다 크다면
        return tree[node] = tree[node * 2] * re_init(node * 2 + 1, mid + 1, end, target) % 1000000007;

    else //찾는 위치가 중앙보다 작거나 같다면 
        return tree[node] = re_init(node * 2, start,mid, target) * tree[node * 2+1] % 1000000007;
